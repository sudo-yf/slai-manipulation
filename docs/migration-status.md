...
| Wujihand client, filters, runtime and safety | `src/robot_teleoperation/wuji/` | `src/slai_mi/devices/wujihand/` | migrated | The fail-closed Python 3.11 / `mediapipe==0.10.21` worker uses the dedicated HBVCAM 4K USB camera selected by `wujihand.retarget_camera_device`; visible 21-landmark input produced a valid 20-DoF target. |
